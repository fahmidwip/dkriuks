...
public function editin_tl() 
{
    ...
    if ($status == 6) {
        foreach ($id_stok as $key => $value) {
            ...
            $stok_stokis = $this->harga_m->get_stok_by_id6($value); // stok milik stokis
...
foreach ($data as $d) {
    ...
    foreach ($items as $item) {
        ...
        $sheet->setCellValue('H' . $row, $firstRow ? $d->tanggal : '');
...
public function export_excel_pisah_khusus() 
{
    $user_id = $this->fungsi->user_login()->stokis;
    $start_date = $this->input->get('start_date');
    $end_date = $this->input->get('end_date');
